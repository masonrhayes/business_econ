...
using Distributions, DataFrames, StatsBase, FreqTables, VegaLite, Pluto, StatsPlots
...
begin
	...
	#error term
	for i in 1:90000
    df[i,"ϵ"] = rand(GeneralizedExtremeValue(0,1,0))
	end
...
for i in 1:90000
    df[i,5] = indicator(df[i,"u"], df[i,"u_maximum"])
end
...
# ╔═╡ 07c3e232-fc77-4387-9f6f-21e9636e704b
for i in 1:90000
    params[i,"α"] = rand(LogNormal(0.3, sqrt(0.10)))
end

# ╔═╡ e2acd5a3-a576-4d1e-9a3b-ef9ac25e6b87
params #checking
...
begin 
	...
	# Rename chosen in df1 and chosen in df2 to chosen1, chosen2 so we can compare
...
sort(q2_df, order(:new_u_maximum))
